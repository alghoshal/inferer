...
def getCurrData():
    ...
    refSentiments = reference.filter(items=["Expert label"]).map(
        lambda x: 0 if x == "bad" else 1)
    reference = reference.filter(
        items=["Generated review"]).join(refSentiments)
    reference.rename(
...
def getRefExpertSentiments(reference):
    ...
    refExpertComments = reference.filter(
        items=["Expert comment"]).to_numpy().flatten()
...
def generateDriftReport(reference, current):
    ...
    runSaveReport(Report([
        ...
        DriftedColumnsCount(cat_stattest="psi", num_stattest="wasserstein", per_column_method={
                            "Label": "psi"}, drift_share=0.5),
    ],
    ...
    runSaveReport(Report([
        ColumnMetricGenerator(ValueDrift, columns=[
                              'LengthReview', 'WordCountReview']),
