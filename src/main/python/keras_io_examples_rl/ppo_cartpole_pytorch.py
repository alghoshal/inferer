"""
PyTorch port of ppo_cartpole.py from keras-io
(@see: https://github.com/keras-team/keras-io/blob/master/examples/rl/ppo_cartpole_pytorch.py)
...
def train_policy(
    ...
    logprobability_buffer = torch.as_tensor(logprobability_buffer)
    with torch.enable_grad():
        ...
        policy_loss = -keras.ops.mean(
            keras.ops.minimum(
                ratio * torch.tensor(advantage_buffer), min_advantage)
        )
...
def train_value_function(observation_buffer, return_buffer):
    return_buffer = torch.as_tensor(return_buffer)
    with torch.enable_grad():
...
policy_optimizer = torch.optim.Adam(
    actor.parameters(), lr=policy_learning_rate)
value_optimizer = torch.optim.Adam(
    critic.parameters(), lr=value_function_learning_rate)
