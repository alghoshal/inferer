...
def testBuildVocab():
    ...
    vocab1 = buildVocabFromRawData(
        raw_train_ds, rebuild=True, saveVocabPath=saveVocabPath)
...
def testBuildModel():
    ...
    model = buildCompileModel(
        train_ds, train_ds, train_ds, max_features, embedding_dim, epochs)
    ...
    assert len(
        model.metrics[1]._user_metrics) == 1 and model.metrics[1]._user_metrics[0] == "accuracy"


def testLoadAndPlotModel():
    model = keras.models.load_model(
        SAVE_TO_DIR+'TextClassificationStudentModel2.5Kc.keras')
...
def testEvaluateModel():
    ...
    model = keras.models.load_model(
        SAVE_TO_DIR+'TextClassificationTorchModel.keras')
...
def testEvaluateLoRaModel():
    loadVocabFromFile(SAVE_TO_DIR+"TextClassificationVocab.pkl")
    model = keras.models.load_model(
        SAVE_TO_DIR+'TextClassificationModelFineTunedLoraExaggerations.keras')
    ...
    vectorzd_text_batch, labelsVectorized = vectorize_text(
        exaggerationsDataSet)
