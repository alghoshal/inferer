...
raw_train_ds = [
    (["This was a great movie!", "Horrible is the word"], np.array([1, 0]))]
...
def callTokenPosnEmbedding():
    ...
    tokenAndPositionEmbedding = TokenAndPositionEmbedding(
        vocabLength, vocabLength, embed_dim)
...
def callRouter():
    ...
    inputs = ops.reshape(tokenPosnEmbeded, [
                         batch_size * num_tokens_per_example, embed_dim])
    router = Router(num_experts, expert_capacity=tcu.vocabLength)
    return router.call(inputs)


def testTokenAndPositionEmbedding():
    assert callTokenPosnEmbedding().shape == (
        len(raw_train_ds[0][0]), tcu.vocabLength, 1, embed_dim)
...
def testSwitch():
    ...
    assert outputSwitch.shape == (
        batch_size, num_tokens_per_example, embed_dim)

    # Simple Switch
    simpleSwitchRoute = SimpleSwitchRoute(
    ...
    assert outputSimpleSwitch.shape == (
        batch_size, num_tokens_per_example, embed_dim)
